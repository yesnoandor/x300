...
import com.android.systemui.statusbar.policy.GpsNotifReceiver;
...
public class PhoneStatusBar extends BaseStatusBar implements DemoMode {
    ...
    GpsNotifReceiver mGpsNotifiReceiver;
	...
	/* zhangzhaolei add for adjust bright & volume toggleSlider 20150720 start */
    View mViewBright;
    View mViewVolumeRing;
    View mViewVolumeCall;
    View mViewVolumeMedia;
	private AudioManager mAudioManager;
	/* zhangzhaolei add for adjust bright & volume toggleSlider 20150720 end */
    ...
    private final Runnable mAutohide = new Runnable() {
		...
		// fleety :: show/hide status-bar by jie.liu add start 
		...
		private BroadcastReceiver needShowStatusBarReceiver = new BroadcastReceiver() {
	
			@Override
			public void onReceive(Context context, Intent intent)
			{
				Log.i(TAG, "[fleety]: needShowStatusBarReceiver...action=" + intent.getAction()+", isShowingStatusBar="+isShowingStatusBar);
				if (ACTION_SHOW_STATUS_BAR.endsWith(intent.getAction()) && !isShowingStatusBar) {
					showStatusBarWindow();
				} else if (ACTION_HIDE_STATUS_BAR.endsWith(intent.getAction()) && isShowingStatusBar) {
					hideStatusBarWindow();
				}
			}
		};
	
		private IntentFilter needShowStatusBarFilter;
		private boolean isShowingStatusBar = false;
		// fleety :: show/hide status-bar by jie.liu add end 
    @Override
    public void start() {
		...
		// fleety :: show/hide status-bar by jie.liu add start  
        needShowStatusBarFilter = new IntentFilter();
        needShowStatusBarFilter.addAction(ACTION_SHOW_STATUS_BAR);
        needShowStatusBarFilter.addAction(ACTION_HIDE_STATUS_BAR);
        mContext.registerReceiver(needShowStatusBarReceiver, needShowStatusBarFilter);		
		// fleety :: show/hide status-bar by jie.liu add end 
    ...
    /* zhangzhaolei add for redesign volume slider 20150827 start */
	...
	private Runnable mSliderRunnable = new Runnable() {
		...
		public void run() {
			...
			h.post(new Runnable() {
                public void run() {
                    if (SystemProperties.getBoolean("ro.config.volume_slider", false)) {
                    ...
                    if (SystemProperties.getBoolean("ro.config.volume_slider", false)) {
                        if (callState == TelephonyManager.CALL_STATE_OFFHOOK) {
                            mViewVolumeCall.setVisibility(View.VISIBLE);
                            mViewVolumeRing.setVisibility(View.GONE);
                            mViewVolumeMedia.setVisibility(View.GONE);
                        } else if (isMusicActive) {
                            mViewVolumeMedia.setVisibility(View.VISIBLE);
                            mViewVolumeRing.setVisibility(View.GONE);
                            mViewVolumeCall.setVisibility(View.GONE);
                        } else {
                            mViewVolumeRing.setVisibility(View.VISIBLE);
                            mViewVolumeCall.setVisibility(View.GONE);
                            mViewVolumeMedia.setVisibility(View.GONE);
                        }
                    }
			...
			mSliderHandler.postDelayed(mSliderRunnable, 500);
		}
	};
	/* zhangzhaolei add for redesign volume slider 20150827 end */
    ...
    protected PhoneStatusBarView makeStatusBarView() {
        ...
        /* zhangzhaolei add for adjust bright & volume toggleSlider 20150720 start */
        mViewBright = (View)mNotificationPanel.findViewById(R.id.include_bright);
        mViewVolumeRing = (View)mNotificationPanel.findViewById(R.id.include_volume_ring);
        mViewVolumeCall = (View)mNotificationPanel.findViewById(R.id.include_volume_call);
        mViewVolumeMedia = (View)mNotificationPanel.findViewById(R.id.include_volume_media);
        if(!SystemProperties.getBoolean("ro.config.volume_slider", false)) {
            mViewBright.setVisibility(View.GONE);
            mViewVolumeRing.setVisibility(View.GONE);
            mViewVolumeCall.setVisibility(View.GONE);
            mViewVolumeMedia.setVisibility(View.GONE);
        }
        ...
        //add for Gps notification
        mGpsNotifiReceiver = new GpsNotifReceiver(mContext);
	...
	// fleety jie.liu add start
    private void showStatusBarWindow() {
        Log.i(TAG, "[fleety]: "+getClass().getSimpleName()+" showStatusBarWindow()");
    ...
    private void hideStatusBarWindow() {
        Log.i(TAG, "[fleety]: "+getClass().getSimpleName()+" hideStatusBarWindow()");
        if (mWindowManager == null) return;
        if (isShowingStatusBar)
        {
