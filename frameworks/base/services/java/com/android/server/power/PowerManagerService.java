...
public final class PowerManagerService extends IPowerManager.Stub
    ...
    public void userActivity(long eventTime, int event, int flags) {
		...
		// Fleety :: Wakeup by activity start 
		/*
    ...
    public void goToSleep(long eventTime, int reason) {
		...
		// Fleety :: Enable Auth start 
        // mContext.enforceCallingOrSelfPermission(android.Manifest.permission.DEVICE_POWER, null);
		// Fleety :: Enable Auth end
    ...
    private void goToSleepInternal(long eventTime, int reason) {
        synchronized (mLock) {
			...
			// goToSleepNoUpdateLocked() : 是goToSleep功能的计算者，来决定是否要休眠
            if (goToSleepNoUpdateLocked(eventTime, reason)) {
				// updatePowerStateLocked() : 是功能的执行者，执行了休眠，而且这个执行者同时负责执行了很多其他的功能，
                updatePowerStateLocked();
            }
    ...
    private void updatePowerStateLocked() {
        ...
        for (;;) {
			...
			// 将系统中所有的wakelock锁更新到一个集合变量mWakeLockSummary中
			// 也就是不管系统中创建了多少个wakelock，一个便足以阻止系统进入睡眠状态
            updateWakeLockSummaryLocked(dirtyPhase1);

			// 在方法中根据系统最后一次调用userActivity()方法的时间计算现在是否可以将屏幕状态的变量mUserActivitySummary置成USER_ACTIVITY_SCREEN_BRIGHT(亮屏)还是USER_ACTIVITY_SCREEN_DIM(暗屏)等
            updateUserActivitySummaryLocked(now, dirtyPhase1);

			// 这个方法是循环结束的关键，如果它的返回值为true，则表示wakefulness的状态发生改变了，降继续循环重新调用前面两个方法更新userActivity和Wakelock集合变量
		...
		// 由PowerManagerService调用到底层的唯一入口
    ...
    public static void lowLevelReboot(String reason) {
    	Log.i(TAG,CommonFunction._FILE_() + ":::" + CommonFunction._FUNC_() + "::+++++++++++++++");
