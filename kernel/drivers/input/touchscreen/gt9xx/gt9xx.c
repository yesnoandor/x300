...
void gtp_touch_enalbe(int enable)
{
	touch_enable = enable;
}
EXPORT_SYMBOL(gtp_touch_enalbe);


//s32 query_fake_sleep(void){
//	return touch_enable;
//}
//EXPORT_SYMBOL(query_fake_sleep);
...
s32 gtp_i2c_read(struct i2c_client *client, u8 *buf, s32 len)
{
	...
	GTP_INFO("I2C Read: 0x%04X, %d bytes.",(((u16)(buf[0] << 8)) | buf[1]), len-2);


	//for(iCnt =0;iCnt<len-2;iCnt++){
	//	GTP_INFO("data[%d]=%x.", iCnt+2, buf[iCnt+2]);
	//}
...
s32 gtp_i2c_write(struct i2c_client *client,u8 *buf,s32 len)
{
	...
	GTP_INFO("I2C Write: 0x%04X, %d bytes.",(((u16)(buf[0] << 8)) | buf[1]), len-2);
...
//GTP_INFO("work_func 4!  data[0]:%d , data[1]:%d, data[2]:%d\n", point_data[0], point_data[1], point_data[2]);
...
//	GTP_INFO("work_func 6!");

    if((finger & 0x80) == 0)
    {
        goto exit_work_func;
    }
//	GTP_INFO("work_func 7!");
...
//	GTP_INFO("work_func 9!");

#if 0	// (GTP_HAVE_TOUCH_KEY || GTP_PEN_HAVE_BUTTON)
 ...
 */
    ...
    {	/* check firmware */
		...
		//printk(KERN_ERR "check firmware, ret:%d, data:%x\n", ret, opr_buf[0]);
	...
	printk(KERN_ERR "check sensor, ret:%d, data:%x\n", ret, sensor_id);
	...
	printk(KERN_ERR "check dtb, ret:%d\n", ret);	
	if (ret < 0) {
...
static void goodix_ts_suspend(struct goodix_ts_data *ts)
{
    ...
    GTP_DEBUG_FUNC();
    if (ts->enter_update) {
		printk(KERN_ERR "##################because updating. so return suspend..\n");
    	return;
    }
...
static void goodix_ts_resume(struct goodix_ts_data *ts)
{
    ...
    if (ts->enter_update) {
		printk(KERN_ERR "##################because updating. so return resume..\n");
    	return;
    }
...
s32 gtp_i2c_read_no_rst(struct i2c_client *client, u8 *buf, s32 len)
{
	...
	GTP_INFO("I2C Read no_rst: 0x%04X, %d bytes.",(((u16)(buf[0] << 8)) | buf[1]), len-2);
...
s32 gtp_i2c_write_no_rst(struct i2c_client *client,u8 *buf,s32 len)
{
	...
	//GTP_INFO("I2C Write no_rst: 0x%04X, %d bytes.",(((u16)(buf[0] << 8)) | buf[1]), len-2);
