...
#define GTP_CHANGE_X2Y        1    //0	   //swap x y
...
#define CTP_CFG_GROUP0 {\
	...
	0x00,0x32,0x1E,0x09,0x05,0x00,0x00,0x00,0x02,0x00,0x00,\
...
// STEP_3(optional): Specify your special config info if needed
  ...
  #define GTP_INT_TRIGGER  1            //#define GTP_INT_TRIGGER  0            // 0: Rising 1: Falling
  #define GTP_CONFIG_OF    1            //chrade add important!!!!!!!!
#else
  #define GTP_MAX_HEIGHT   800
  #define GTP_MAX_WIDTH    480
...
#define GTP_INFO(fmt,arg...)            do{\
                                         if(GTP_DEBUG_ON)\
                                         printk(KERN_ERR "<<-GTP-INFO->> "fmt"\n", ##arg);\
                                       }while(0)

#define GTP_ERROR(fmt,arg...)          printk(KERN_ERR "<<-GTP-ERROR->> "fmt"\n",##arg)
#define GTP_DEBUG(fmt,arg...)          do{\
                                         if(GTP_DEBUG_ON)\
                                         printk(KERN_ERR "<<-GTP-DEBUG->> [%d]"fmt"\n",__LINE__, ##arg);\
                                       }while(0)
#define GTP_DEBUG_ARRAY(array, num)    do{\
                                         ...
                                         {\
                                            printk(KERN_ERR "<<-GTP-DEBUG-ARRAY->>\n");\
                                            for (i = 0; i < (num); i++)\
                                            {\
                                                printk("%02x   ", (a)[i]);\
                                                if ((i + 1 ) %10 == 0)\
...
#define GTP_DEBUG_FUNC()               do{\
                                         ...
                                         printk(KERN_ERR "<<-GTP-FUNC->> Func:%s@Line:%d\n",__func__,__LINE__);\
                                       }while(0)
