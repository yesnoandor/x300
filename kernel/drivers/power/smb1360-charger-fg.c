...
static int smb1360_get_prop_batt_capacity(struct smb1360_chip *chip)
{
	...
	/*sunhaiqiang modify: our smb map 100% to 4350mv, for 4200 it is 86%, so remap 100% to 86% = 4200mv*/
	pr_debug("msys_soc_reg=0x%02x, fg_soc=%d batt_full = %d", reg,
						soc, chip->batt_full);
				       ...
				       union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_HEALTH:
		...
		// Fleety : Bypass power state start
		val->intval = POWER_SUPPLY_HEALTH_GOOD;
		// Fleety : Bypass power state end
		break;
