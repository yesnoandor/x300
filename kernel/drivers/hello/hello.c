...
static ssize_t hello_proc_read(char* page,  char **start, off_t off, int count, int* eof, void* data) {
    ...
    val = __hello_get_val(hello_dev, page);
...
static void hello_create_proc(void) {
...
#if 0
	entry = proc_create(HELLO_DEVICE_PROC_NAME, 0x666, NULL, NULL);
	if (!entry){
		printk(KERN_ALERT"Failed to alloc hello_proc_dev./n");
		return;   
	}
	...
	#if 0
	struct proc_dir_entry *entry;
	proc_create(HELLO_DEVICE_PROC_NAME, S_IRUGO, NULL, seq_fops);
...
static int __init hello_init(void){ 
    ...
    printk(KERN_INFO "MAJOR Dev = %d \r\n",hello_major); 
    printk(KERN_INFO "MINOR Dev = %d \r\n",hello_minor);

    /*分配helo设备结构体变量*/
    hello_dev = (struct hello_android_dev*)kmalloc(sizeof(struct hello_android_dev), GFP_KERNEL);
    if(!hello_dev) {
        err = -ENOMEM;
