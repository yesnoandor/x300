...
#define DEBUG_FLEETY_CPUFREQ	"FLEETY_CPUFREQ"

#ifdef	DEBUG_FLEETY_CPUFREQ
...
static int msm_cpufreq_init(struct cpufreq_policy *policy)
{
	...
	dprintk(KERN_INFO "%s:%s::+++++++++++++++\r\n",DEBUG_FLEETY_CPUFREQ,__FUNCTION__);
	...
	if (cpufreq_frequency_table_cpuinfo(policy, table))
	{
		...
		dprintk(KERN_ERR "cpufreq: failed to get policy min/max\n");
	}
	...
	dprintk(KERN_INFO "cpufreq: cpu%d init at %d switching to %d\r\n",policy->cpu, cur_freq, table[index].frequency);
