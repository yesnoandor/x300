...
#define DEBUG_FLEETY_CODEC	"FLEETY_CODEC"

#ifdef	DEBUG_FLEETY_CODEC
...
static const DECLARE_TLV_DB_SCALE(digital_gain, 0, 1, 1);
...
#if 1
static void mute_work_fn(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct msm8x16_wcd_priv *msm8x16_wcd = container_of(dwork, struct msm8x16_wcd_priv,
												mute_work);

	int mute;
	//int stb;
	
	dprintk(KERN_ERR "%s:%s::+++++++++++++++\r\n",DEBUG_FLEETY_CODEC,__FUNCTION__);

	mute = msm8x16_wcd->ex_amp_mute;
	
	dprintk(KERN_ERR "mute = %d\r\n",mute);
	//dprintk(KERN_ERR "dwork = 0x%x\r\n",dwork);

	if(mute)
	{
		//fleety_amp_standby(mute);
		//fleety_amp_mute(mute);
		//mdelay(200);
		//fleety_amp_standby(mute);
	}
	else
...
static int msm_ext_spk_pa_mode = PA_MODE_2; //default mode 2 for AW8155
...
static const struct soc_enum msm_ext_spk_pa_mode_enum[] = {
		SOC_ENUM_SINGLE_EXT(5, msm_ext_spk_pa_mode_text),
};
#endif
					    ...
					    bool turn_on)
{
	...
	//dprintk(KERN_ERR "turn_on : %d count: %d\r\n",turn_on,count);
			      ...
			      struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:		// before widget power up
...
#ifdef CONFIG_AUDIO_EXT_SPK_PA
		//dprintk(KERN_ERR "spk_hp_switch = %d\r\n",spk_hp_switch);
		...
		if(spk_hp_switch == MSM_HP_PATH) {
			...
			if (!strncmp(w->name, "HPHL PA", 8)) {
				...
				if(ext_spk_pa_gpio >= 0) {
					for(i = 0; i < msm_ext_spk_pa_mode; i++) {
						gpio_direction_output(ext_spk_pa_gpio, 0);
						udelay(2);
				...
				dev_info(codec->dev, "%s: PA mode(%d) enabled\n", __func__, msm_ext_spk_pa_mode);
			}
		}
#endif
		break;
	...
	case SND_SOC_DAPM_POST_PMD:	// after widget power down
		...
		if ((spk_hp_switch >= MSM_SPK_PATH) && (!strncmp(w->name, "HPHL PA", 8))) {
			if(ext_spk_pa_gpio >= 0)
				gpio_direction_output(ext_spk_pa_gpio, 0);
			msm_ext_spk_pa_mode = PA_MODE_2;
			msleep(20);
			if(headset_switch_gpio >= 0)
				gpio_direction_output(headset_switch_gpio, 1);
			dev_info(codec->dev, "%s: PA disabled\n", __func__);
		}
#endif
		dev_dbg(codec->dev,
		...
		struct snd_soc_dai *dai)
{
	...
	//dprintk(KERN_ERR "substream = %s  stream = %d\r\n",substream->name, substream->stream);
	...
	if(substream->stream == AIF1_PB)
		fleety_amp_standby(0);
...
int msm8x16_wcd_digital_mute(struct snd_soc_dai *dai, int mute)
{
	...
	if(dai->id == AIF1_PB)
	{
		...
		//	dprintk(KERN_ERR "delay 5s\r\n");

		//	usleep_range(50000000, 51001000);
		
			//fleety_amp_standby(mute);

			msm8x16_wcd->ex_amp_stb = mute;
			msm8x16_wcd->ex_amp_mute = mute;
			schedule_delayed_work(&msm8x16_wcd->mute_work,
								msecs_to_jiffies(5));
		}
		else
		{
			...
			//usleep_range(20000000, 21001000);
			fleety_amp_mute(mute);
...
static int msm8x16_wcd_codec_probe(struct snd_soc_codec *codec)
{
	...
	// init delayed mute work queue
	INIT_DELAYED_WORK(&msm8x16_wcd_priv->mute_work, mute_work_fn);
